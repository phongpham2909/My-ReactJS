...
class SectionContentRight extends Component {
  constructor() {
    super();
    this.state = {
      currentPage: 1,
  ...
  componentDidMount(){
    const { match: {
      ...
      this.props.GetProductsFollowCategory(id);
  }
  componentDidUpdate(prevProps) {
    ...
    if (id !== prevProps.match.match.params.id) {
      this.props.GetProductsFollowCategory(id);
    }
  }
    ...
    render() {
      const { currentPage, todosPerPage } = this.state;
        const { classes, productsFollowCategory } = this.props;
         // Logic for displaying current todos
         const indexOfLastTodo = currentPage * todosPerPage;
         const indexOfFirstTodo = indexOfLastTodo - todosPerPage;
         const currentTodos = productsFollowCategory.slice(indexOfFirstTodo, indexOfLastTodo);
        ...
        const pageNumbers = [];
        for (let i = 1; i <= Math.ceil(productsFollowCategory.length / todosPerPage); i++) {
          pageNumbers.push(i);
        }
...
export default withStyles(styles)(SectionContentRight);